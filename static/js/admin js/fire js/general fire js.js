...
(function () {
	...
	App.els = {
		...
		closeIncidentDetailsModal: document.getElementById(
	...
	App.data = {
		simulatedNotifications: [
			{
				id: "NOTIF-001",
				messageTitle: "High Severity Fire",
				messageSubtitle: "Yola Main Market",
				details: "Incident FR-AD001 reported.",
			...
			},
			{
				...
				messageSubtitle: "Jimeta Modern Market",
				details: "Incident FR-AD006 requires attention.",
			...
			},
			{
				...
				messageSubtitle: "Gombi Farmland",
				details: "Incident FR-AD004 has been resolved.",
			...
			},
			{
				...
				messageSubtitle: "Federal Poly Mubi",
				details: "Incident FR-AD005 flagged as duplicate.",
			...
			},
			{
				...
				messageSubtitle: "Dashboard Maintenance",
	...
	App.renderNotificationsToUI = function (notifications) {
		if (!App.els.notificationList || !App.els.notificationDot) {
			if (App.els.notificationList)
				App.els.notificationList.innerHTML = `<div class="text-center p-4 text-slate-400">Error loading notifications.</div>`;
			return;
		}
	...
	App.populateDateFilters = function () {
		if (
			...
			return;
		const years = new Set(
			App.data.allFireIncidents.map((inc) =>
	...
	App.populateIncidentsTable = function (incidents) {
		...
		incidents.forEach((incident) => {
			const row = `<tr><td>${incident.id}</td><td>${
				incident.location
			}</td><td><span class="severity-text-${incident.severity.toLowerCase()}">${
				incident.severity
			}</span></td><td><span class="status-badge status-${incident.status.toLowerCase()}">${
				incident.status
			}</span></td><td>${new Date(
				incident.dateTime
			).toLocaleString()}</td><td><button class="view-details-btn bg-red-600 hover:bg-red-700 text-white font-bold py-1 px-3 rounded-lg text-xs" data-incident-id="${
				incident.id
			}">View</button></td></tr>`;
			App.els.allIncidentsTableBody.insertAdjacentHTML("beforeend", row);
		});
		App.els.allIncidentsTableBody
	...
	App.openIncidentDetailsModal = function (incidentId) {
		...
		const incident = App.data.allFireIncidents.find(
			(inc) => inc.id === incidentId
		);
		if (!incident) return;
		if (App.els.detailId) App.els.detailId.textContent = incident.id;
		if (App.els.detailLocation)
			App.els.detailLocation.textContent = incident.location;
		if (App.els.detailSeverity)
			App.els.detailSeverity.textContent = incident.severity;
		if (App.els.detailStatus)
			App.els.detailStatus.textContent = incident.status;
		if (App.els.detailDateTime)
			App.els.detailDateTime.textContent = new Date(
				incident.dateTime
			).toLocaleString();
		if (App.els.detailCause) App.els.detailCause.textContent = incident.cause;
		if (App.els.detailIsDuplicate)
			App.els.detailIsDuplicate.textContent = incident.isDuplicate
				? "Yes"
				: "No";
	...
	function initGeneralEventListeners() {
		...
		if (App.els.closeNotificationBtn)
			App.els.closeNotificationBtn.addEventListener("click", () => {
				if (App.els.notificationDropdown)
					App.els.notificationDropdown.classList.add("hidden");
				if (App.els.notificationButton)
					App.els.notificationButton.classList.remove("active");
			});
		document.addEventListener("click", (event) => {
			const clickedInsideAnyModal = event.target?.closest(
		...
		if (App.els.modalSearchInput)
			App.els.modalSearchInput.addEventListener(
				"input",
				App.applyFiltersToAllIncidents
			);
		if (App.els.closeIncidentDetailsModal)
			App.els.closeIncidentDetailsModal.addEventListener("click", () => {
				if (App.els.incidentDetailsModal)
					App.els.incidentDetailsModal.classList.add("hidden");
			});
		if (App.els.closeDetailsButton)
			App.els.closeDetailsButton.addEventListener("click", () => {
				if (App.els.incidentDetailsModal)
					App.els.incidentDetailsModal.classList.add("hidden");
			});
	}
