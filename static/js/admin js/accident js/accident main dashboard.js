...
(function() {
    ...
    App.data = {
        ...
        allAccidentIncidents: [
            { id: 'ACC-001', location: 'Lagos-Ibadan Expressway', impact: 'Fatal', status: 'Pending', dateTime: '2025-06-20T10:00:00Z', cause: 'Speeding', isDuplicate: false },
            { id: 'ACC-002', location: 'Third Mainland Bridge', impact: 'Severe', status: 'Resolved', dateTime: '2025-06-18T14:30:00Z', cause: 'Break Failure', isDuplicate: false },
            { id: 'ACC-003', location: 'Apapa-Oshodi Expressway', impact: 'Minor', status: 'Investigating', dateTime: '2025-06-15T09:00:00Z', cause: 'Reckless Driving', isDuplicate: false },
            { id: 'ACC-004', location: 'Ojuelegba Bridge', impact: 'Fatal', status: 'Pending', dateTime: new Date(Date.now() - 2 * 3600 * 1000).toISOString(), cause: 'Container Fall', isDuplicate: false },
            { id: 'ACC-005', location: 'Badagry Expressway', impact: 'Severe', status: 'Resolved', dateTime: new Date(Date.now() - 24 * 3600 * 1000).toISOString(), cause: 'Pothole', isDuplicate: true }, 
            { id: 'ACC-006', location: 'Eko Bridge', impact: 'Minor', status: 'Pending', dateTime: new Date(Date.now() - 5 * 24 * 3600 * 1000).toISOString(), cause: 'Tyre Burst', isDuplicate: false },
            { id: 'ACC-007', location: 'Victoria Island, Ahmadu Bello Way', impact: 'Minor', status: 'Resolved', dateTime: '2025-05-30T11:00:00Z', cause: 'Drink Driving', isDuplicate: false },
            { id: 'ACC-008', location: 'Lekki-Epe Expressway', impact: 'Severe', status: 'Investigating', dateTime: '2025-05-25T16:00:00Z', cause: 'Unknown', isDuplicate: true }, 
            { id: 'ACC-009', location: 'Ikorodu Road', impact: 'Fatal', status: 'Resolved', dateTime: '2025-04-10T08:00:00Z', cause: 'Over-speeding', isDuplicate: false },
            { id: 'ACC-010', location: 'Abuja-Kaduna Road', impact: 'Fatal', status: 'Pending', dateTime: '2025-01-05T12:00:00Z', cause: 'Head-on Collision', isDuplicate: false },
            { id: 'ACC-011', location: 'Kano-Zaria Road', impact: 'Severe', status: 'Resolved', dateTime: '2025-01-10T15:00:00Z', cause: 'Lost Control', isDuplicate: false },
            { id: 'ACC-012', location: 'Port Harcourt-Owerri Road', impact: 'Minor', status: 'Investigating', dateTime: '2025-02-01T07:30:00Z', cause: 'Animal Crossing', isDuplicate: false },
            { id: 'ACC-013', location: 'Enugu-Onitsha Expressway', impact: 'Fatal', status: 'Pending', dateTime: '2025-02-14T20:00:00Z', cause: 'Overloading', isDuplicate: true },
            { id: 'ACC-014', location: 'Benin-Ore Road', impact: 'Severe', status: 'Resolved', dateTime: '2025-03-01T06:00:00Z', cause: 'Bad Road', isDuplicate: false },
            { id: 'ACC-015', location: 'Calabar-Uyo Road', impact: 'Minor', status: 'Resolved', dateTime: '2025-03-18T10:00:00Z', cause: 'Distracted Driving', isDuplicate: false },
            { id: 'ACC-016', location: 'Kaduna Bypass', impact: 'Fatal', status: 'Pending', dateTime: '2025-04-01T13:00:00Z', cause: 'Speeding', isDuplicate: false },
            { id: 'ACC-017', location: 'Abeokuta-Sagamu Road', impact: 'Severe', status: 'Investigating', dateTime: '2025-04-12T17:00:00Z', cause: 'Break Failure', isDuplicate: true },
            { id: 'ACC-018', location: 'Ibadan Ring Road', impact: 'Minor', status: 'Resolved', dateTime: '2025-05-01T09:00:00Z', cause: 'Reckless Driving', isDuplicate: false },
        ],
 
        youtubeVideos: [
            { id: 'S1-x1Fm-QJg', title: 'Safe Driving Practices', description: 'Tips for defensive driving and avoiding common road hazards.', category: 'Driving Safety' },
            { id: 'vA6z7o0_mXs', title: 'Pedestrian Safety Awareness', description: 'Important rules and tips for pedestrians to stay safe on the roads.', category: 'Pedestrian Safety' },
            { id: 'f5w0M45M_t8', title: 'First Aid for Road Accidents', description: 'Basic first aid steps to take when responding to a road accident victim.', category: 'Emergency Response' },
            { id: 'UfC08Krxr0g', title: 'Motorcycle Safety Gear Guide', description: 'A comprehensive guide to essential safety gear for motorcycle riders.', category: 'Motorcycle Safety' },
        ]
    };
    ...
    async function getActiveIncidents() {
        return new Promise(r => setTimeout(() => r(
            App.data.allAccidentIncidents.filter(inc => inc.status !== 'Resolved')
        ), App.data.SIMULATED_DELAY));
    }
    ...
    async function loadDashboardStats() {
        ...
        if(App.els.responseRateMessage1 && App.els.responseRateMessage2) {
            if (stats.responseRate.current >= stats.responseRate.target) {
                ...
                App.els.responseRateMessage1.textContent = "Current accident response rate is good, keep it up!";
    ...
    async function initMonthlyChart() {
        if (!App.els.monthlyChartContainer || !App.els.monthlyIncidentsChart || typeof Chart === 'undefined') return;
        ...
        monthlyChartInstance = new Chart(App.els.monthlyIncidentsChart.getContext('2d'), {
            type: 'bar', data: data,
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: true, labels: { color: '#e2e8f0', font: { size: 12 } } }, tooltip: { callbacks: { label: (c) => `${c.dataset.label}: ${c.parsed.y} incidents` } } }, scales: { x: { grid: { display: false }, ticks: { color: '#a0aec0', font: { size: 12 } } }, y: { beginAtZero: true, grid: { color: '#2d3748' }, ticks: { color: '#a0aec0', font: { size: 12 } } } } }
        });
    }

    function getStatisticsCounts(incidents) {
        let fatal = 0, severe = 0, minor = 0, duplicateReports = 0;
    ...
    async function renderStatisticsChart(selectedMonth, selectedYear) {
        if (!App.els.statisticsChartContainer || !App.els.statisticsChart || !App.els.noDataStatisticsMessage || typeof Chart === 'undefined') return;
        ...
        const { fatal, severe, minor, duplicateReports } = getStatisticsCounts(filteredIncidents);
        ...
        if (!hasData) {
            ...
            const chartData = {
                ...
                datasets: [{
                    ...
                    borderColor: '#1a202c', borderWidth: 2,
                }]
            };
            if (statisticsChartInstance) statisticsChartInstance.destroy();
            statisticsChartInstance = new Chart(App.els.statisticsChart.getContext('2d'), {
                type: 'doughnut', data: chartData,
                options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { color: '#e2e8f0', boxWidth: 14, padding: 20, font: { size: 12 } } }, tooltip: { callbacks: { label: (c) => `${c.label || ''}: ${c.parsed !== null ? c.parsed + ' incidents' : ''}` } } } }
            });
        }
    ...
    function populateStatisticsFilters() {
        ...
        ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"].forEach((name, index) => {
            App.els.statisticsMonthFilter.innerHTML += `<option value="${index + 1}">${name}</option>`;
        });
        const currentYear = new Date().getFullYear();
        const yearsInData = new Set(App.data.allAccidentIncidents.map(inc => new Date(inc.dateTime).getFullYear()));
    ...
    async function renderActiveIncidentsTable() {
        ...
        filteredIncidents.forEach(incident => {
            const impactClass = incident.impact === 'Fatal' ? 'text-red-500' : (incident.impact === 'Severe' ? 'text-orange-500' : 'text-green-500');
            const row = `<tr><td class="px-4 py-3 text-sm text-white">${incident.id}</td><td class="px-4 py-3 text-sm text-white">${incident.location}</td><td class="px-4 py-3 text-sm font-semibold ${impactClass}">${incident.impact}</td><td class="px-4 py-3"><span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full status-${incident.status.toLowerCase()}">${incident.status}</span></td></tr>`;
    ...
    function populateCategoryDropdown() {
        ...
        const categories = new Set(App.data.youtubeVideos.map(v => v.category).filter(Boolean));
    ...
    function renderExistingVideos(searchTerm = '') {
        ...
        const filteredVideos = App.data.youtubeVideos.filter(v => 
        ...
        App.els.existingVideosList.innerHTML = filteredVideos.length === 0 
    ...
    async function handleAddUpdateVideo(event) {
        ...
        if (editingVideoId) {
            const index = App.data.youtubeVideos.findIndex(v => v.id === editingVideoId);
            if (index !== -1) {
    ...
    function confirmDeleteVideo(videoId) {
        ...
        newCancelBtn.onclick = () => App.els.confirmationModal.classList.add('hidden');
    }

    async function deleteVideo(videoId) {
        const videoTitle = App.data.youtubeVideos.find(v => v.id === videoId)?.title || 'Unknown';
        App.data.youtubeVideos = App.data.youtubeVideos.filter(v => v.id !== videoId);
    ...
    function initDashboardEventListeners() {
        App.els.activeStatusFilter?.addEventListener('change', renderActiveIncidentsTable);
        App.els.activeImpactFilter?.addEventListener('change', renderActiveIncidentsTable);
        ...
        App.els.videoSearchInput?.addEventListener('input', (e) => renderExistingVideos(e.target.value));

        const handleStatsFilterChange = () => renderStatisticsChart(App.els.statisticsMonthFilter.value, App.els.statisticsYearFilter.value);
        App.els.statisticsMonthFilter?.addEventListener('change', handleStatsFilterChange);
        App.els.statisticsYearFilter?.addEventListener('change', handleStatsFilterChange);
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if(typeof App.loadNotifications === 'function') {
            ...
            console.error("loadNotifications function not found on App object. Is base.js loaded?");
        }
    });
